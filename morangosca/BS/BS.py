		...
		while 1:
			...
			if (requestUser == 'AUT'):
					...
					while (i < num_files):
						...
						while(left_size > 0):
							if(left_size < 256 or left_size == 256):
								dataUPL= connUser.recv(left_size) #receive data from file
							else:
								dataUPL= connUser.recv(256) #receive data from file
							f.write(dataUPL)
							left_size = left_size - len(dataUPL)
				...
				try:
					...
					user_dir = cwd + '/user_' + current_user
		...
		if(main_comand == "RGR"):
			...
			try:
				...
				if(os.path.exists(cwd)):
					...
					nFiles = len(list_files)
					element = 'LFD ' + BSNAME + ' ' + str(BSPORT) + ' ' + str(nFiles)  #reply
					print "element: " + element
					
					sockBS.sendto(element, server_address) #LFD portBS IPBS N 
