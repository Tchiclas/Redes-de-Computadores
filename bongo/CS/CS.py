...
def nextWordUDP(sock1):
	word = ''
	while 1:
		char,addr = sock1.recvfrom(1)
		if((char == ' ' or char =='\n') and word != ''):
			break
		if(not(word == '' and char == ' ')):
			word = word + char
...
def parseTCP(conn):
				...
				if(not len(data) == 3+4*n_files):
					...
					if (user_dir in list_dir):
						if(directory in os.listdir(user_dir_path)):
							...
							sockBCK.recv(4096)
							tag = nextWordUDP(sockBCK)
							ip = nextWordUDP(sockBCK)
							port = nextWordUDP(sockBCK)
							n_files = nextWordUDP(sockBCK)

							i = 0
							reply = []
							while (i < n_files):
								reply[i*n] = nextWordUDP(sockBCK)
								reply[i*n + 1] = nextWordUDP(sockBCK)
								reply[i*n + 2] = nextWordUDP(sockBCK)
								reply[i*n + 3] = int(nextWordUDP(sockBCK))
							...
							answer = []
							new_n_files = 0
							print "list_ files: " + str(list_files)
							while i < length:
	...
	#IPBS portBS				
		...
		if(len(data) != 2):
			conn.sendall("RSR ERR\n")
		else:
			cwd = os.getcwd()
			user_dir = '/user_' + username
			user_dir_path = cwd + user_dir
			directory = data[1]
			dir_path = user_dir_path + '/' + directory
			if (os.path.isdir(dir_path)):
				file = open(dir_path + '/IP_port.txt', 'rb')
				file_data = file.read() #portnum portname
				line = file_data.split()
				IPBS = line[1]
				portBS = line[0]
				#devia verificar se ipbs e portbs ainda existem?
				file.close()
				message = 'RSR ' + IPBS + ' ' + portBS + '\n'
				conn.sendall(message)
			else:
				conn.sendall("RSR EOF\n")
		print "RST"
		...
		try:
			...
			print 'ERR: socket time out'
			print e
			sockBS.close()
			conn.sendall("LFD NOK\n") #time out user request
			return
